...
fees. The fee is real — it is just captured differently.
...
```
Victim receives 327.241 aEthAAVE — 4.064 less than 
...
comes directly from the Sync log — no external source.
...
order size by any reasonable safety factor, reject 
...
**Five on-chain checks to distinguish accidental 
from deliberate:**
...
deeper. Sophisticated actors can exploit this — 
submit intents through CoW, let solvers execute, 
on-chain trail shows bot activity not human wallet. 
...
CoW advertises zero fees — the fee is real, just 
